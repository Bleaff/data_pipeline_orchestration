...
class FakePipelineServiceManager:
    ...
    Mirrors `start_pipeline`/`stop_pipeline`/`status`/`pipelines` exactly, so the API
    layer can be exercised at full speed; a slower true-subprocess integration test
    lives in `test_app_integration.py` for the one path worth proving end-to-end.
    """
