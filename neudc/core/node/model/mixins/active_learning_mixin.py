...
class ActiveLearningMixin:
    """
    Collects frames coming from different folders, waits until every folder
    (i.e. video / dataset chunk) has delivered all its images, and then
    selects a subset for annotation with an *uncertainty x diversity* strategy.
    """

    def __init__(
        self, num_to_select: int = 20, conf_strategy: str = "sum", conf_weight: float = 1.0
    ) -> ActiveLearningMixin:
        """
        Parameters
        ----------
        num_to_select : int
            How many images to pick once all expected frames are gathered.
        conf_strategy : {"sum", "avg", "max"}
            How to aggregate box-confidence values into a single “uncertainty”
            number per image.
        conf_weight : float
            Relative importance of uncertainty vs. embedding distance when the
    ...
    def active_select(self, frames: Batch) -> Batch:
        """
        ...
        embs = np.vstack([f.embedding for f in frames])
        ...
        for i, f in enumerate(frames):
            ...
            if confs.size == 0:
                ...
                if self.conf_strategy == "sum":
                    ...
                    raise ValueError(f"Unknown conf_strategy {self.conf_strategy}")
        if np.ptp(uncert) > 1e-8:
