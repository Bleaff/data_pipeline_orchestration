"""Threaded node wiring `CreateDatasetMixin` frame/label persistence into the pipeline."""
...
class CreateDataset(CreateDatasetMixin, BaseThreadedNode):
    """A threaded processing node that saves incoming frames and YOLO-format annotations to disk.
    ...
    def __init__(self, mailbox: Any, save_dir: str) -> None:
        """Initialize the CreateDataset node.
    ...
    @classmethod
    def from_config(cls: type[CreateDataset], config: dict[str, Any]) -> CreateDataset:
        """Build a CreateDataset instance from a configuration dictionary.
