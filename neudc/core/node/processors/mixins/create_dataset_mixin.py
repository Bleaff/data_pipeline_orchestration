"""Mixin implementing frame-to-disk persistence for dataset-creation nodes."""
...
class CreateDatasetMixin:
    ...
    Frames are written to a specified dataset directory. Intended to be combined with
    processing nodes that receive frames during runtime.
    ...
    def __init__(self, save_dir: str | Path, *args: Any, **kwargs: Any) -> None:
        """Initialize the mixin with the base save directory.
    ...
    def process(self, frame: Frame) -> Frame:
        """Save the frame's image and its YOLO-format label file, then return it unchanged.
    ...
    def get_output_dirs(self, frame: Frame) -> tuple[Path, Path]:
        """Construct image and label directory paths for a frame, based on its source folder name.
    ...
    def save_image(self, frame: Frame, image_path: Path) -> None:
        """Save the image data from the frame to disk.
    ...
    def save_labels(self, frame: Frame, label_path: Path) -> None:
        """Save detection boxes from the frame to a label file in YOLO format.
        ...
        with Path(label_path).open("w") as f:
            if frame.boxes:
